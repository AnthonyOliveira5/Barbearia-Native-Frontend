...
import { AlertCircle, Camera, CheckCircle2, Edit2, LogOut, Mail, Phone, User } from 'lucide-react-native';
    ...
    <Modal transparent visible={visible} animationType="fade">
      <View className="flex-1 bg-black/60 justify-center items-center px-6">
        <View className="bg-white w-full max-w-sm rounded-2xl p-6 items-center shadow-2xl">
          ...
          <Text className="text-xl font-bold text-gray-900 mb-2 text-center">{type === 'confirm' ? 'Sair da Conta' : type === 'success' ? 'Sucesso' : 'Atenção'}</Text>
          ...
          {type === 'confirm' ? (
            <View className="flex-row gap-3 w-full">
              ...
              <Pressable onPress={onConfirm} className="flex-1 py-3 rounded-xl items-center bg-red-500"><Text className="text-white font-bold">Sair Agora</Text></Pressable>
            </View>
          ) : (
...
export default function PerfilScreen() {
  ...
  const handleSave = async () => {
    if (!name.trim()) { setModalConfig({ visible: true, type: 'error', message: "Nome é obrigatório." }); return; }
    try {
      setLoading(true);
  ...
  const toggleEdit = () => {
    if (isEditing) { setName(user?.name || ''); setTelefone(user?.telefone || ''); }
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
          ...
          <View className="bg-white pb-8 pt-6 rounded-b-[32px] shadow-sm mb-6 items-center px-6">
            <View className="relative">
              ...
              {/* Botão de Câmera com Ação */}
              <Pressable onPress={handleUpdateAvatar} className="absolute bottom-0 right-0 bg-yellow-400 p-2 rounded-full border-2 border-white shadow-sm">
            ...
            <View className="flex-row justify-between items-center mb-4">
              ...
              {!isEditing && (
                <Pressable onPress={toggleEdit} className="flex-row items-center gap-1">
                  <Edit2 size={16} color="#EAB308" /><Text className="text-yellow-600 font-bold">Editar</Text>
                </Pressable>
              )}
            </View>
            <View className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 gap-4">
              <View><Text className="text-xs text-gray-400 font-bold uppercase mb-1 ml-1">Nome</Text><View className={`flex-row items-center bg-gray-50 border rounded-xl px-3 py-3 ${isEditing ? 'border-yellow-400 bg-white' : 'border-gray-100'}`}><User size={20} color={isEditing ? "#000" : "#9CA3AF"} /><TextInput value={name} onChangeText={setName} editable={isEditing} className={`flex-1 ml-3 text-base ${isEditing ? 'text-gray-900' : 'text-gray-500'}`} /></View></View>
              <View><Text className="text-xs text-gray-400 font-bold uppercase mb-1 ml-1">Telefone</Text><View className={`flex-row items-center bg-gray-50 border rounded-xl px-3 py-3 ${isEditing ? 'border-yellow-400 bg-white' : 'border-gray-100'}`}><Phone size={20} color={isEditing ? "#000" : "#9CA3AF"} /><TextInput value={telefone} onChangeText={setTelefone} editable={isEditing} keyboardType="phone-pad" className={`flex-1 ml-3 text-base ${isEditing ? 'text-gray-900' : 'text-gray-500'}`} /></View></View>
              <View><Text className="text-xs text-gray-400 font-bold uppercase mb-1 ml-1">E-mail</Text><View className="flex-row items-center bg-gray-100 border border-gray-100 rounded-xl px-3 py-3 opacity-70"><Mail size={20} color="#9CA3AF" /><Text className="flex-1 ml-3 text-base text-gray-500">{email}</Text></View></View>
              {isEditing && (
                <View className="flex-row gap-3 mt-2">
                  ...
                  <Pressable onPress={handleSave} disabled={loading} className="flex-1 bg-yellow-400 py-3 rounded-xl items-center">{loading ? <ActivityIndicator color="black" /> : <Text className="font-bold text-black">Salvar</Text>}</Pressable>
                </View>
              )}
            </View>
          </View>
          ...
          <View className="px-6 mt-8">
            <Pressable onPress={requestSignOut} className="flex-row items-center justify-center gap-2 bg-red-50 border border-red-100 p-4 rounded-xl active:bg-red-100">
              <LogOut size={20} color="#EF4444" /><Text className="text-red-500 font-bold text-lg">Sair da Conta</Text>
            </Pressable>
